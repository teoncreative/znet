...
namespace znet {
  ...
  enum class InetProtocolVersion {
    IPv4,
    IPv6
  };
  ...
  class InetAddress {
  public:
    InetAddress(InetProtocolVersion ipv, std::string readable) : ipv_(ipv), readable_(std::move(readable)) { }

    ZNET_NODISCARD virtual const std::string& readable() const { return readable_; };
  ...
  public:
    explicit InetAddressIPv4(PortType port) : InetAddress(InetProtocolVersion::IPv4, "") {
    ...
    InetAddressIPv4(IPv4Type ip, PortType port) : InetAddress(InetProtocolVersion::IPv4, "") {
      addr.sin_addr.s_addr = INADDR_ANY;
      addr.sin_family = AF_INET;
      addr.sin_port = htons(port);
    ...
    InetAddressIPv4(const std::string& str, PortType port) : InetAddress(InetProtocolVersion::IPv4, "") {
      addr.sin_addr.s_addr = INADDR_ANY;
      addr.sin_family = AF_INET;
      addr.sin_port = htons(port);
    ...
    ZNET_NODISCARD sockaddr* handle_ptr() const override { return (sockaddr*) &addr; }
  ...
  public:
    explicit InetAddressIPv6(PortType port) : InetAddress(InetProtocolVersion::IPv6, "") {
    ...
    InetAddressIPv6(IPv6Type ip, PortType port) : InetAddress(InetProtocolVersion::IPv6, "") {
      addr.sin6_len = sizeof(sockaddr_in6);
      addr.sin6_family = AF_INET6;
      addr.sin6_flowinfo = 0;
      addr.sin6_port = htons(port);
    ...
    InetAddressIPv6(const std::string& str, PortType port) : InetAddress(InetProtocolVersion::IPv6, "") {
      addr.sin6_len = sizeof(sockaddr_in6);
      addr.sin6_family = AF_INET6;
      addr.sin6_flowinfo = 0;
      addr.sin6_port = htons(port);
