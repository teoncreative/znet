...
namespace znet {
  ServerSession::ServerSession(Ref<InetAddress> local_address, Ref<InetAddress> remote_address, int client_socket) : local_address_(std::move(local_address)), remote_address_(std::move(remote_address)), client_socket_(client_socket) {
    is_alive_ = true;
  ...
  void ServerSession::Process() {
    ...
    data_size_ = recv(client_socket_, buffer_, sizeof(buffer_), 0);
    if (data_size_ > MAX_BUFFER_SIZE) {
      Close();
      LOG_ERROR("Received data bigger than maximum buffer size (rx: {}, max: {}), closing connection!", data_size_, MAX_BUFFER_SIZE);
      return;
    }
    if (data_size_ == 0) {
      Close();
