...
//
// Jolt rigid body state on the wire.
//
...
// guarantee.
//
// everything lives in namespace znet::ext::jolt, because the readers here and
...
inline void WritePosition(BitWriter& bits, const JPH::Vec3& value,
                          const BodyQuantization& q) {
  bits.WriteFloatRanged(value.GetX(), q.position_min, q.position_max, q.position_bits);
  bits.WriteFloatRanged(value.GetY(), q.position_min, q.position_max, q.position_bits);
  bits.WriteFloatRanged(value.GetZ(), q.position_min, q.position_max, q.position_bits);
}

/** @brief Reads a position written by WritePosition. */
inline JPH::Vec3 ReadPosition(BitReader& bits, const BodyQuantization& q) {
  const float x =
 ...
 */
inline void WriteOrientation(BitWriter& bits, const JPH::Quat& value) {
  quant::Quat4 neutral;
  neutral.x = value.GetX();
  neutral.y = value.GetY();
  neutral.z = value.GetZ();
  neutral.w = value.GetW();
 ...
 */
inline JPH::Quat ReadOrientation(BitReader& bits) {
...
inline void WriteTransform(BitWriter& bits, const JPH::Vec3& position,
...
inline void ReadTransform(BitReader& bits, JPH::Vec3& position,
                          JPH::Quat& orientation, const BodyQuantization& q) {
...
inline void WriteLinearVelocity(BitWriter& bits, const JPH::Vec3& value,
                                const BodyQuantization& q) {
  bits.WriteFloatRanged(value.GetX(), -q.linear_speed_max, q.linear_speed_max,
                        q.linear_speed_bits);
  bits.WriteFloatRanged(value.GetY(), -q.linear_speed_max, q.linear_speed_max,
                        q.linear_speed_bits);
  bits.WriteFloatRanged(value.GetZ(), -q.linear_speed_max, q.linear_speed_max,
                        q.linear_speed_bits);
}

/** @brief Reads a linear velocity written by WriteLinearVelocity. */
inline JPH::Vec3 ReadLinearVelocity(BitReader& bits, const BodyQuantization& q) {
...
inline void WriteAngularVelocity(BitWriter& bits, const JPH::Vec3& value,
                                 const BodyQuantization& q) {
  bits.WriteFloatRanged(value.GetX(), -q.angular_speed_max, q.angular_speed_max,
                        q.angular_speed_bits);
  bits.WriteFloatRanged(value.GetY(), -q.angular_speed_max, q.angular_speed_max,
                        q.angular_speed_bits);
  bits.WriteFloatRanged(value.GetZ(), -q.angular_speed_max, q.angular_speed_max,
                        q.angular_speed_bits);
}

/** @brief Reads an angular velocity written by WriteAngularVelocity. */
inline JPH::Vec3 ReadAngularVelocity(BitReader& bits, const BodyQuantization& q) {
